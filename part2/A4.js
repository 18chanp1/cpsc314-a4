...
const skyboxTexture = skyboxLoader.load([
  "images/skybox/posx.jpeg",
  "images/skybox/negx.jpeg",
  "images/skybox/posy.jpeg",
  "images/skybox/negy.jpeg",
  "images/skybox/posz.jpeg",
  "images/skybox/negz.jpeg"
]);
...
const envmapMaterial = new THREE.ShaderMaterial({
  uniforms: {
    ...
    skybox: {type: "t", value: skyboxTexture}
  }
});
...
let object1;
{
  ...
  gltfLoader1.load(
    ...
    function ( gltf ) {
      ...
      object1.position.set(0.0, -53.6, -8.0);
...
let object;
{
  ...
  gltfLoader.load(gltfFileName, (gltf) => {
    ...
    object.position.set(0.0, -58, -8.0);
    scene.add( object );
  });
}

const terrainGeometry = new THREE.BoxGeometry(5000, 5000, 5);
const terrain = new THREE.Mesh(terrainGeometry, floorMaterial);
terrain.position.y = -60;
terrain.rotation.set(- Math.PI / 2, 0, -0.1);
// scene.add(terrain);

loadAndPlaceOBJ('gltf/armadillo.obj', envmapMaterial, function (armadillo) {
  armadillo.position.set(0.0, -53.6, 6.0);
...
// add the car
// const cam = new THREE.TextureLoader().load( 'gltf/car/textures/Material_294_baseColor.png' );
...
let Carpos1;
carloader.load('gltf/car/old_rusty_car.glb', function ( gltf ) {
  let ambientLight = new THREE.AmbientLight(0xffffff, 3.0);
  gltf.scene.add(ambientLight);
  gltf.scene.scale.set(0.1, 0.1, 0.1);
  gltf.scene.rotation.y = Math.PI / 2;
  gltf.scene.position.set(20.0, -57, -20);
  gltf.parent = worldFrame;
  Carpos1 = gltf.scene.position;
...
carloader.load('gltf/car/old_rusty_car.glb', function ( gltf ) {
  ...
  gltf.scene.scale.set(0.1, 0.1, 0.1);
  gltf.scene.rotation.y = -Math.PI / 2;
  gltf.scene.position.set(20.0, -57, -40);
  gltf.parent = worldFrame;
  Carpos2 = gltf.scene.position;
  scene.add( gltf.scene );
})
...
function update() {
  ...
  ticks = ticks > 1000 ? -1000: ticks + 2;

  if(Carpos1 != undefined) Carpos1.set(ticks, -57, -60);
  if(Carpos2 != undefined) Carpos2.set(-ticks, -57, -100);
